 ...
 * 设置 Bot 命令菜单
 */
export async function setBotCommands(botToken, commands, chatId, userId = null) {
...
export async function sendTelegramMessage(botToken, chatId, text, replyToMessageId = null, parseMode = 'HTML') {
	...
	try {
		...
		if (!response.ok) {
			...
			const { message_id } = await sendTelegramMessageMarkdownV2(botToken, chatId, markdownv2Text, replyToMessageId); //  使用 MarkdownV2 重新发送
...
async function sendTelegramMessageMarkdownV2(botToken, chatId, text, replyToMessageId = null) {
	...
	const payload = {
		...
		parse_mode: 'Markdown', //  !!!  强制使用 MarkdownV2 解析模式  !!!
	};
