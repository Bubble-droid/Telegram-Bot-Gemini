...
export async function isGroupInCooldown(
	...
	//  !!!  通用冷却 Key  !!!
	const lastRequestTimestampKey = `cooldown:${chatId}`;
	...
	if (elapsedMs < cooldownMs) {
		...
		console.log(`群组 ${chatId} 冷却中，剩余 ${remainingSeconds} 秒`); //  !!!  区分日志：通用冷却  !!!

		const replyText = `⏱️ 系统正在冷却中，请等待 ${remainingSeconds} 秒后重试！`; //  构建冷却提示消息
		const { message_id: messageId } = await sendTelegramMessage(botToken, chatId, replyText, replyToMessageId, 'HTML');

		if (messageId) {
			await scheduleDeletion(env, botToken, chatId, messageId, 30 * 1000);
		}
